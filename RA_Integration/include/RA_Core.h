...
// A lot of my changes were erasesd by blindly resolving conflicts to "theirs"
// not a big deal
...
// This ifdef seems redundant since we use the STL, but yeah it should be C linkage
_EXTERN_C
...
_DEPRECATED API int  CCONV _RA_HTTPGetRequestExists(
    _UNUSED const char* sPageName) noexcept;
...
API void CCONV _RA_InstallSharedFunctions(bool(CCONV* fpIsActive)(),
    void(CCONV* fpCauseUnpause)(), void(CCONV* fpRebuildMenu)(),
    void(CCONV* fpEstimateTitle)(char*), void(CCONV* fpResetEmulation)(),
    void(CCONV* fpLoadROM)(const char*)) noexcept;

// Got a warning that this function has a high chance of throwing
// A possible solution is make a member function version for it to do exception
// handling then export it as a C function like here
// C5039: A throwing C function, will cause undefined behavior if it throws
...
// won't be null now
extern char* _MallocAndBulkReadFileToBuffer(const char* sFilename, std::streamsize nFileSizeOut);
...
extern void _FetchGameHashLibraryFromWeb() noexcept;
extern void _FetchGameTitlesFromWeb() noexcept;
extern void _FetchMyProgressFromWeb() noexcept;
...
// Stuff down here was to address repetition, typing the same thing out 1000 times is tedious

// we could make some these  into a class instead but w/e
// GetActiveWindow returns the handle for the active window (owner/parent) or null if there is no window active
...
>
int CALLBACK
show_error(const StringType& msg, HWND hwnd = GetActiveWindow()) noexcept
{
    return MessageBox(hwnd, TEXT(msg.c_str()), TEXT("Error!"),
        MB_OK | MB_ICONERROR);
...
>
int CALLBACK
show_success(const StringType& msg, HWND hwnd = GetActiveWindow()) noexcept
{
    return MessageBox(hwnd, TEXT(msg.c_str()), TEXT("Success!"),
...
>
int CALLBACK
show_confirmation(const StringType& msg, HWND hwnd = GetActiveWindow()) noexcept
{
    return MessageBox(hwnd, TEXT(msg.c_str()), TEXT("Are you sure?"),
...
>
int CALLBACK
show_info(const StringType& msg, HWND hwnd = GetActiveWindow()) noexcept
{
    return MessageBox(hwnd, TEXT(msg.c_str()), TEXT("Info"),
        MB_OK | MB_ICONINFORMATION);
} // end function show_error
...
>
int CALLBACK
show_warning(const StringType& msg, HWND hwnd = GetActiveWindow()) noexcept
{
    return MessageBox(hwnd, TEXT(msg.c_str()), TEXT("Warning!"),
        MB_OK | MB_ICONWARNING);
} // end function show_error

// Shoot, non-template functions have to be seperated from declarations
