...
BOOL RAWeb::DoBlockingHttpPost( const std::string& sRequestedPage, const std::string& sPostString, DataStream& ResponseOut )
{
    ...
    // There's gotta be something I changed that was revereted
		...
		if( hConnect != nullptr )
		{
			...
			if( hRequest != nullptr )
			{
				...
				if( bSendSuccess && WinHttpReceiveResponse( hRequest, nullptr ) )
				{
					...
					while( nBytesToRead > 0 )
					{
						DataStream pData; // this thing corrupted the response for achievement reporter, changed to "DataStream"
						pData.reserve(nBytesToRead);
						{
							DWORD nBytesFetched = 0;
							if( WinHttpReadData( hRequest, pData.data(), nBytesToRead, &nBytesFetched ) )
							{
								ASSERT( nBytesToRead == nBytesFetched );
                                // what in gods name happend here?
                                // I need to see the old one
								pData = pData.c_str(); // need to see how it looks
                                // ok another approach
                                // Ok it was able to parse it this time

                                // Need to see if it's always the same amount
								size_t a = nBytesToRead - sizeof(pData.data()) + 1;
                                // There's always 4 garabage characters, lets see if this will help
								ResponseOut.append(pData.c_str(), nBytesToRead);
							}
							else
							{
								RA_LOG( "Assumed timed out connection?!" );
	...
	if( ResponseOut.size() > 0 )
	{
		...
		if( doc.HasParseError() )
		{
            ...
            // .... Keeps on saying Root not singular
