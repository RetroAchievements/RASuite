...
HBITMAP LoadLocalPNG( const std::string& sPath, const RASize& sz )
{
	...
	// may the image can get decoded now
	...
	if( SUCCEEDED( hr ) )
	{
		// Cuz... Interfaces can't be nullified
		// Intefaces are used by the Operating system the API will handle it
    ...
    CComPtr<IWICBitmapSource> pToRenderBitmapSource{ null };
	if( SUCCEEDED( hr ) )
