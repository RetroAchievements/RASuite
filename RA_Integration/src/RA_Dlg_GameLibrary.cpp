...
void Dlg_GameLibrary::ParseGameHashLibraryFromFile(GameHashLib& gamehash_libary)
{
	...
	doc.ParseStream(isw); // I guess because it expects an lvalue but not an rvalue
...
void Dlg_GameLibrary::SetupColumns()
{
	//	Remove all columns,
	...
	for (auto& i : titles)
	{
		...
		tstring sCol{ titles.at(static_cast<ColumnTitles::size_type>(pos)) };	//	scoped cache

		// based off the struct
		LV_COLUMN col{
			UINT{LVCF_TEXT | LVCF_WIDTH | LVCF_SUBITEM | LVCF_FMT},
			LVCFMT_LEFT | LVCFMT_FIXED_WIDTH,
			widths.at(static_cast<ColumnTitles::size_type>(pos)),
			sCol.data(),
			255,
			pos
	...
	DataStream& sROMRawData, std::string& sAbsFileDir)
{
	if (hROMReader != INVALID_HANDLE_VALUE)
	{
		...
		{
			nSize = static_cast<int>((File_Inf.nFileSizeHigh << 16) +
				File_Inf.nFileSizeLow);
		} // end if

		auto nBytes{ 0_dw };
		_UNUSED auto bResult{ ReadFile(hROMReader,
			reinterpret_cast<LPVOID>(sROMRawData.data()),
			static_cast<DWORD>(nSize), &nBytes, nullptr) };

		const auto sHashOut{
			RAGenerateMD5(sROMRawData.c_str(), static_cast<size_t>(nSize))
		};
...
void Dlg_GameLibrary::ReloadGameListData()
{
	...
	tstring sRomDir;
	sRomDir.reserve(to_unsigned(len));
...
bool Dlg_GameLibrary::LaunchSelected()
{
	if (const auto nSel{ ListView_GetSelectionMark(m_hList) }; nSel != -1)
	{
		...
		tstring buffer;
		buffer.reserve(to_unsigned(len));
...
// NO FUCKING WONDER!
//HWND Dlg_GameLibrary::Create()
//{
//	bIsModal = false;
//
//
//	m_hGameLibWnd = CreateDialog(g_hThisDLLInst, MAKEINTRESOURCE(nResourceId),
//		g_RAMainWnd, MsgQueue);
//
//	if (!m_hGameLibWnd)
//		throw std::system_error{ GetLastErrorCode(), GetLastErrorMsg() };
//
//	return m_hGameLibWnd;
//}
BOOL Dlg_GameLibrary::OnInitDialog(HWND hwnd, _UNUSED HWND hwndFocus, 
...
void Dlg_GameLibrary::OnCommand(HWND hwnd, int id, 
	_UNUSED HWND hwndCtl, _UNUSED UINT codeNotify)
{
...
void Dlg_GameLibrary::OnLbxGamelist()
{
	...
	if (nSel != -1)
	{
		...
		tstring buffer;
		buffer.reserve(to_unsigned(length));
...
void Dlg_GameLibrary::OnTimer(HWND hwnd, _UNUSED UINT id)
{
...
INT_PTR Dlg_GameLibrary::DialogProc(_UNUSED HWND hwnd, _UNUSED UINT uMsg,
