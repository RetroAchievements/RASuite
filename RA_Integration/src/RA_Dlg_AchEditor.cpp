...
INT_PTR Dlg_AchievementEditor::AchievementEditorProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
	{
		...
		//	For scanning changes to achievement Dirty_Conditions (hit counts)
	...
	case WM_COMMAND:
		...
		case IDC_RA_CHK_ACH_ACTIVE:
			if (ActiveAchievement() != NULL)
			{
				SendMessage(g_AchievementsDialog.GetWindow(), WM_COMMAND, IDC_RA_RESET_ACH, NULL);
		...
		case IDC_RA_ACH_TITLE:
		{
			...
			case EN_CHANGE:
			{
				...
				HWND hList = GetDlgItem(g_AchievementsDialog.GetWindow(), IDC_RA_LISTACHIEVEMENTS);
				...
				if (nSelectedIndex != -1)
				{
					...
					if (GetDlgItemText(hDlg, IDC_RA_ACH_TITLE, buffer, 1024))
					{
						...
						g_AchievementsDialog.OnEditData(g_pActiveAchievements->GetAchievementIndex(*pActiveAch), ach_col::Title, pActiveAch->Title());
					}
				}
			}
			break;
	...
	if (m_pSelectedAchievement != NULL)
	{
		if (m_pSelectedAchievement->BadgeImageURI().compare(sNewName) != 0)
		{
			...
			if (g_nActiveAchievementSet == Core)
			{
				...
				g_AchievementsDialog.OnEditData(nOffs, ach_col::Modified, "Yes");
			}
		}
	}
