...
BOOL ConditionSet::Test( BOOL& bDirtyConditions, BOOL& bResetRead, BOOL bMatchAny )
{
	...
	//	Now, read all Pause Dirty_Conditions, and if any are true, do not process further (retain old state)
	...
	//	Read all standard Dirty_Conditions, and process as normal:
	for( i = 0; i < nNumConditions; ++i )
	{
		Condition* pNextCond = &m_Conditions[ i ];
	...
	//	Now, ONLY read reset Dirty_Conditions!
	for( i = 0; i < nNumConditions; ++i )
	{
		Condition* pNextCond = &m_Conditions[ i ];
		if( pNextCond->IsResetCondition() )
		{
			...
			if( bConditionValid )
			{
				...
				break;						//	No point processing any further reset Dirty_Conditions.
			}
		}
	}
