...
/// <summary>
/// Another RapdiJSON change -SyrianBallaS
/// </summary>
/// <returns></returns>
API void CCONV _RA_LoadPreferences()
{
	...
	SetCurrentDirectory(Widen(g_sHomeDir).c_str());
	FILE* fp{ fopen(std::string(std::string(RA_PREFERENCES_FILENAME_PREFIX) + g_sClientName + ".cfg").c_str(), "rb") };
	...
	else
	{
		...
		FileReadStream is{ fp,rBuffer,sizeof(rBuffer) };
...
/// <summary>
/// More RapidJSON changes -SyrianBallaS
/// </summary>
/// <returns></returns>
API void CCONV _RA_SavePreferences()
{
	...
	SetCurrentDirectory(Widen(g_sHomeDir).c_str());
	FILE* pf{ fopen(std::string(std::string(RA_PREFERENCES_FILENAME_PREFIX) + g_sClientName + ".cfg").c_str(), "wb") };

	if (pf != NULL)
	{
		char wBuffer[BUFSIZE];
		FileWriteStream os{ pf,wBuffer,sizeof(wBuffer) };
...
/// <param name="pOffsetInOut"></param>
/// <param name="bTerminate"></param>
/// <returns></returns>
...
{
	SetCurrentDirectory(Widen(g_sHomeDir).c_str());
	FILE* pf = fopen(sFileName.c_str(), "wb");
	if (pf == 0)
	{
		char wBuffer[BUFSIZE];
		FileWriteStream os{ pf,wBuffer,sizeof(wBuffer) };
